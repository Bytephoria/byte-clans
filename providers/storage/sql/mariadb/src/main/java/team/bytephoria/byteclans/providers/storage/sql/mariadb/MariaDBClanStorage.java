...
public final class MariaDBClanStorage extends AbstractSQLClanStorage {

    private final ExecutorService executorService;
    public MariaDBClanStorage(
    ...
    public void create(final @NotNull ClanEntry clanEntry) {
        try (
            ...
            preparedStatement.setObject(1, clanEntry.clanUniqueId());
            preparedStatement.setString(2, clanEntry.ownerName());
            preparedStatement.setObject(3, clanEntry.ownerUniqueId());
    ...
    public void update(final @NotNull ClanEntry clanEntry) {
        try (
            ...
            preparedStatement.setObject(2, clanEntry.ownerUniqueId());
            ...
            preparedStatement.setObject(14, clanEntry.clanUniqueId());
            preparedStatement.executeUpdate();
    ...
    public void update(final @NotNull ClanEntry clanEntry, final @NotNull ClanField @NotNull ... fields) {
        ...
        try (
            ...
            for (final ClanField field : fields) {
                switch (field) {
                    case OWNER_UNIQUE_ID -> preparedStatement.setObject(index++, clanEntry.ownerUniqueId());
            ...
            preparedStatement.setObject(index, clanEntry.clanUniqueId());
            preparedStatement.executeUpdate();
        } catch (SQLException e) {
            this.logger().log(Level.WARNING, e.getMessage(), e);
        }
    ...
    public @NotNull Async async() {
        return new Async() {
            ...
            private MariaDBClanStorage instance() {
                return MariaDBClanStorage.this;
            }
