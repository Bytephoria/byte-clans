...
**ByteClans** is a lightweight, extensible clan javaPlugin for **PaperMC** servers.  
...
The javaPlugin ships with the essentials — clan creation, member management, invite system, chat modes, PvP settings and statistics — while staying lightweight and easy to extend.
...
ByteClans is designed to be a **general-purpose**  clan javaPlugin that works on any server, regardless of game mode.
Pull requests that push the javaPlugin toward a specific game mode — such as clan homes, territory claiming, economy integration, or similar — will be automatically rejected.
If you need those kinds of features, you have two supported paths:

- **Create an addon** that depends on ByteClans. The javaPlugin provides a stable API specifically for extending functionality without bloating the core.
